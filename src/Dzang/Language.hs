...
parseExpr env@(Env _ ds pf) = parseOperand env >>= \res -> peek >>= \case
  ...
  Just c
    | (c == ' ' && pf) || c == ')' -> return . head . operands $ printAll env
      { operands = res : ds
      }
...
popAndPrint env@(Env ('+' : os) (rhs : lhs : ds) _) =
  env { operators = os, operands = Add lhs rhs : ds }
popAndPrint env@(Env ('-' : os) (rhs : lhs : ds) _) =
  env { operators = os, operands = Sub lhs rhs : ds }
popAndPrint env@(Env ('/' : os) (rhs : lhs : ds) _) =
  env { operators = os, operands = Div lhs rhs : ds }
popAndPrint env@(Env ('*' : os) (rhs : lhs : ds) _) =
  env { operators = os, operands = Mul lhs rhs : ds }
popAndPrint env@(Env (' ' : os) (rhs : lhs : ds) _) =
  env { operators = os, operands = Application lhs rhs : ds }
