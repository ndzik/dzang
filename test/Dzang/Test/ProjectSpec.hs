...
spec = describe "Dzang Project" $ do
  ...
  it "parses valid projects on filesystem" testFileProjects
...
testFileProjects = do
  ...
  where
    testFileProject pd =
      readModules pd >>= \fp ->
        parseProject (Project "TestProject" fp)
          `shouldBe` [
                       Right $
                         Module
                           "TestModule"
